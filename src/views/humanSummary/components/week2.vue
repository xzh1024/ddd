        ...
        methods: {
            ...
            calcAdvanceOrDelay(item) {
                ...
                if(Number(item.schedule_start_timestamp) < firstDate && Number(item.schedule_end_timestamp) < firstDate){
                    ...
                    //正常开始
                    item.startX = this.findStartIndex(Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) * this.cellWidth;
                    item.taskStartX = this.findStartIndex(Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) * this.cellWidth;
                    if(Number(item.schedule_end_timestamp) === Number(item.end_timestamp)){
                        //正常结束
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.advanceEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.end_timestamp,'end'))) / 86400) * this.cellWidth;
                        item.firstTime = this.coverDateTime(item.schedule_start_timestamp,'dateTime');
                        item.lastTime = this.coverDateTime(item.schedule_end_timestamp,'dateTime');
                    }else if(Number(item.end_timestamp) > Number(item.schedule_end_timestamp)){
                        //延期结束
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.delayEndX = Math.ceil((Number(this.calcTimeFrame(item.end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_end_timestamp,'end'))) / 86400) * this.cellWidth;
                        item.firstTime = this.coverDateTime(item.schedule_start_timestamp,'dateTime');
                    ...
                    //提前开始
                    item.startX = this.findStartIndex(Number(this.calcTimeFrame(item.start_timestamp,'start'))) * this.cellWidth;
                    item.taskStartX = this.findStartIndex(Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) * this.cellWidth;
                    item.advanceStartX = Math.ceil((Number(this.calcTimeFrame(item.schedule_start_timestamp,'start')) - Number(this.calcTimeFrame(item.start_timestamp,'start'))) / 86400) * this.cellWidth;
                    if(Number(item.schedule_end_timestamp) === Number(item.end_timestamp)){
                        //正常结束
                        item.stageType = 3;
                        item.stageTypeStr = '提前开始,正常结束';
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.end_timestamp,'end')) - Number(this.calcTimeFrame(item.start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.delayEndX = Math.ceil((Number(this.calcTimeFrame(item.end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_end_timestamp,'end'))) / 86400) * this.cellWidth;
                        item.firstTime = this.coverDateTime(item.start_timestamp,'dateTime');
                        item.lastTime = this.coverDateTime(item.end_timestamp,'dateTime');
                    ...
                    //延期开始
                    item.startX = this.findStartIndex(Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) * this.cellWidth;
                    item.taskStartX = this.findStartIndex(Number(this.calcTimeFrame(item.start_timestamp,'start'))) * this.cellWidth;
                    item.delayStartX = Math.ceil((Number(this.calcTimeFrame(item.start_timestamp,'start')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                    if(Number(item.schedule_end_timestamp) === Number(item.end_timestamp)){
                        //正常结束
                        item.stageType = 6;
                        item.stageTypeStr = '延期开始,正常结束';
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.firstTime = this.coverDateTime(item.schedule_start_timestamp,'dateTime');
                        item.lastTime = this.coverDateTime(item.schedule_end_timestamp,'dateTime');
                    }else if(Number(item.end_timestamp) < Number(item.schedule_end_timestamp)){
                        //提前结束
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.end_timestamp,'end')) - Number(this.calcTimeFrame(item.start_timestamp,'start'))) / 86400) * this.cellWidth;
                        item.advanceEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.end_timestamp,'end'))) / 86400) * this.cellWidth;
                        item.firstTime = this.coverDateTime(item.schedule_start_timestamp,'dateTime');
                        item.lastTime = this.coverDateTime(item.schedule_end_timestamp,'dateTime');
                    }else if(Number(item.end_timestamp) > Number(item.schedule_end_timestamp)){
                        ...
                        if(Number(item.start_timestamp) >= Number(item.schedule_end_timestamp)){
                            item.taskEndX = 0;
                            item.delayEndX = Math.ceil((Number(this.calcTimeFrame(item.end_timestamp,'end')) - Number(this.calcTimeFrame(item.start_timestamp,'start'))) / 86400) * this.cellWidth;
                        }else {
                            item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_timestamp,'end')) - Number(this.calcTimeFrame(item.start_timestamp,'start'))) / 86400) * this.cellWidth;
                            item.delayEndX = Math.ceil((Number(this.calcTimeFrame(item.end_timestamp,'end')) - Number(this.calcTimeFrame(item.schedule_end_timestamp,'end'))) / 86400) * this.cellWidth;
                        }
