    ...
    export default {
        ...
        methods:{
                    ...
                    this.calcTaskCoordinate(data).then((dataArr)=>{
                        if(dataArr.length > 0){
                            dataArr.forEach(function (item,index) {
                                ...
                                // item.notStartFlag = item.status === 201 || item.status === 102;
                                //计算任务的开始，结束坐标
            ...
            draw(ctx,dataArr){
                ...
                if(dataArr && dataArr.length > 0){
                    dataArr.forEach(function (item) {
                        ...
                        //画任务
                        if(item.status === 103 || item.status === 202 || item.status === 205){
                            ...
                            if('node' in item ){
                                ...
                                if(!Boolean(item.end_timestamp)){
                                    if(item.status === 106 || item.status === 205){
                                        //状态为已逾期的 用后端服务器时间计算
                                        item.end_timestamp = this.currentDateTime.timestamp;
                                        item.end_time = this.currentDateTime.dateTime;
                                    }else {
            ...
            calcAdvanceOrDelay(item){
                ...
                let lastDate = dateTime[dateLength-1].end + (6*86400); //当前时间段最后一个时间 + 6天的时间 因为是已周为刻度
                if(Number(item.start_timestamp) < Number(item.schedule_start_timestamp) && Number(item.end_timestamp) < Number(item.schedule_end_timestamp)){
                    //提前开始,提前结束
                    let startIndex = _this.findStartIndex(item.start_timestamp);
                    let startDay = _this.dateTime[startIndex].begin;
                    let afterDay = (Number(item.start_timestamp) - startDay)/86400;

                    if(Number(item.start_timestamp) <= firstDate && Number(item.schedule_end_timestamp) <= lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.schedule_end_timestamp) - firstDate)/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - firstDate)/86400;
                        advanceEndDay = (Number(item.schedule_end_timestamp) - Number(item.end_timestamp))/86400;
                    }else if(Number(item.start_timestamp) >= firstDate && Number(item.schedule_end_timestamp) >= lastDate){
                        //尾超 头不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((lastDate - Number(item.start_timestamp))/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - Number(item.start_timestamp))/86400;
                        advanceEndDay = (lastDate - Number(item.end_timestamp))/86400;
                    }else if(Number(item.start_timestamp) <= firstDate && Number(item.schedule_end_timestamp) >= lastDate){
                        //头超 尾超
                        item.startX = 0;
                        item.endX = Math.ceil((((lastDate - firstDate)/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - firstDate)/86400;
                        advanceEndDay = (lastDate - Number(item.end_timestamp))/86400;
                    }else {
                        //都不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((Number(item.schedule_end_timestamp) - Number(item.start_timestamp))/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - Number(item.start_timestamp))/86400;
                        advanceEndDay = (Number(item.schedule_end_timestamp) - Number(item.end_timestamp))/86400;
                    }

                    item.advanceStartX = advanceStartDay * _this.dayWidth;
                    item.advanceEndX = advanceEndDay * _this.dayWidth;
                    item.stageType = 1;
                    item.stageTypeStr = '提前开始,提前结束';

                    item.firstTime = _this.coverDateTime(item.start_timestamp,'dateTime');
                    item.lastTime = _this.coverDateTime(item.schedule_end_timestamp,'dateTime');
                    // console.log(item);
                    // console.log('提前开始:'+advanceStartDay);
                    // console.log('提前结束:'+advanceEndDay);
                }else if(Number(item.start_timestamp) < Number(item.schedule_start_timestamp) && Number(item.end_timestamp) > Number(item.schedule_end_timestamp)){
                    //提前开始,延期结束
                    let startIndex = _this.findStartIndex(item.start_timestamp);
                    let startDay = _this.dateTime[startIndex].begin;
                    let afterDay = (Number(item.start_timestamp) - startDay)/86400;
                    if(Number(item.start_timestamp) <= firstDate && Number(item.end_timestamp) <=lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.end_timestamp) - firstDate)/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - firstDate)/86400;
                        delayEndDay = (Number(item.end_timestamp) - Number(item.schedule_end_timestamp))/86400;
                    }else if(Number(item.start_timestamp) >= firstDate && Number(item.end_timestamp) >= lastDate){
                        //尾超 头不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((lastDate - Number(item.start_timestamp))/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - Number(item.start_timestamp))/86400;
                        delayEndDay = (lastDate - Number(item.schedule_end_timestamp))/86400;
                    }else if(Number(item.start_timestamp) <= firstDate && Number(item.end_timestamp) >= lastDate){
                        //头超 尾超
                        item.startX = 0;
                        item.endX = Math.ceil((((lastDate - firstDate)/86400)) * _this.dayWidth);
                        ...
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((Number(item.end_timestamp) - Number(item.start_timestamp))/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - Number(item.start_timestamp))/86400;
                        delayEndDay = (Number(item.end_timestamp) - Number(item.schedule_end_timestamp))/86400;
                    }

                    item.advanceStartX = advanceStartDay * _this.dayWidth;
                    item.delayEndX = delayEndDay * _this.dayWidth;
                    item.stageType = 2;
                    item.stageTypeStr = '提前开始,延期结束';
                    item.firstTime = _this.coverDateTime(item.start_timestamp,'dateTime');
                    item.lastTime = _this.coverDateTime(item.end_timestamp,'dateTime');
                    // console.log(item);
                    // console.log('提前开始:'+advanceStartDay);
                    ...
                    //延期开始,延期结束
                    let startIndex = _this.findStartIndex(item.schedule_start_timestamp);
                    let startDay = _this.dateTime[startIndex].begin;
                    let afterDay = (Number(item.schedule_start_timestamp) - startDay)/86400;

                    if(Number(item.schedule_start_timestamp) <= firstDate && Number(item.end_timestamp) <= lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.end_timestamp) - firstDate)/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_timestamp) - firstDate)/86400;
                        delayEndDay = (Number(item.end_timestamp) - Number(item.schedule_end_timestamp))/86400;
                    }else if(Number(item.schedule_start_timestamp) >= firstDate && Number(item.end_timestamp) >= lastDate){
                        //尾超 头不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((lastDate - Number(item.schedule_start_timestamp))/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_timestamp) - Number(item.schedule_start_timestamp))/86400;
                        delayEndDay = (lastDate - Number(item.schedule_end_timestamp))/86400;
                    }else if(Number(item.schedule_start_timestamp) <= firstDate && Number(item.end_timestamp) >= lastDate){
                        //头超 尾超
                        item.startX = 0;
                        ...
                        //都不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((Number(item.end_timestamp) - Number(item.schedule_start_timestamp))/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_timestamp) - Number(item.schedule_start_timestamp))/86400;
                        delayEndDay = (Number(item.end_timestamp) - Number(item.schedule_end_timestamp))/86400;
                    }

                    item.delayStartX = delayStartDay * _this.dayWidth;
                    item.delayEndX = delayEndDay * _this.dayWidth;
                    item.stageType = 4;
                    item.stageTypeStr = '延期开始,延期结束';
                    item.firstTime = _this.coverDateTime(item.schedule_start_timestamp,'dateTime');
                    item.lastTime = _this.coverDateTime(item.end_timestamp,'dateTime');
                    // console.log(item);
                    ...
                    if(Number(item.schedule_start_timestamp) <= firstDate && Number(item.schedule_end_timestamp) <= lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.schedule_end_timestamp) - firstDate)/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_timestamp) - firstDate)/86400;
                        advanceEndDay = (Number(item.schedule_end_timestamp) - Number(item.end_timestamp))/86400;
                    }else if(Number(item.schedule_start_timestamp) >= firstDate && Number(item.schedule_end_timestamp) >= lastDate){
                        //尾超 头不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((lastDate - Number(item.schedule_start_timestamp))/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_timestamp) - Number(item.schedule_start_timestamp))/86400;
                        ...
                        //都不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((Number(item.schedule_end_timestamp) - Number(item.schedule_start_timestamp))/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_timestamp) - Number(item.schedule_start_timestamp))/86400;
                        advanceEndDay = (Number(item.schedule_end_timestamp) - Number(item.end_timestamp))/86400;
                    }
                    item.delayStartX = delayStartDay * _this.dayWidth;
            ...
            gotoTask(task){
                ...
                if(translateX > taskLeft){
                    let moveLeft = 0;
                    if (taskLeft > 0){
                        moveLeft = translateX - (translateX - taskLeft) - this.dayWidth;
                    } else {
                    ...
                    if(surplusWidth>0){
                        moveLeft = translateX + (taskLeft - translateX - viewWidth + taskWidth) + this.dayWidth;
                    }else {
