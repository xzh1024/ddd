    ...
    export default {
        ...
        methods: {
            ...
            calcAdvanceOrDelay(item) {
                ...
                if(Number(item.start_timestamp) <= Number(item.schedule_start_timestamp) && Number(item.end_timestamp) <= Number(item.schedule_start_timestamp)){
                    //实际开始时间小于排期开始时间
                    //实际结束时间小于排期开始时间
                    item.startX = _this.findStartIndex(item.start_timestamp) * _this.cellWidth;
                    item.endX = Math.ceil((((Number(item.end_timestamp) - Number(item.start_timestamp)) / 86400)) * _this.cellWidth);
                    item.stageType = 0;
                    ...
                    //提前开始,提前结束
                    if(Number(item.start_timestamp) > lastDate && Number(item.schedule_end_timestamp) > lastDate){
                        //头尾都在最后时间外
                        let exceededStartDay = Math.ceil((Number(item.start_timestamp) - lastDate) / 86400);
                        ...
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.schedule_end_timestamp) - firstDate) / 86400)) * _this.cellWidth);
                        ...
                        //尾超 头不超
                        item.startX = _this.findStartIndex(item.start_timestamp) * _this.cellWidth;
                        item.endX = Math.ceil((((lastDate - Number(item.start_timestamp)) / 86400)) * _this.cellWidth);
                        ...
                        //都不超
                        item.startX = _this.findStartIndex(item.start_timestamp) * _this.cellWidth;
                        item.endX = Math.ceil((((Number(item.schedule_end_timestamp) - Number(item.start_timestamp)) / 86400)) * _this.cellWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - Number(item.start_timestamp)) / 86400;
                        advanceEndDay = (Number(item.schedule_end_timestamp) - Number(item.end_timestamp)) / 86400;
                    }
                    ...
                    //提前开始,延期结束
                    if(Number(item.start_timestamp) > lastDate && Number(item.end_timestamp) > lastDate){
                        //头尾都在最后时间外
                        let exceededStartDay = Math.ceil((Number(item.start_timestamp) - lastDate) / 86400);
                        let exceededEndDay = Math.ceil((Number(item.end_timestamp) - lastDate) / 86400);
                        item.startX = (_this.findStartIndex(lastDate) + exceededStartDay) * _this.cellWidth;
                        item.endX = (_this.findStartIndex(lastDate) + exceededEndDay) * _this.cellWidth;
                    } else if(Number(item.start_timestamp) <= firstDate && Number(item.end_timestamp) <= lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.end_timestamp) - firstDate) / 86400)) * _this.cellWidth);
                        advanceStartDay = (Number(item.schedule_start_timestamp) - firstDate) / 86400;
                    ...
                    //延期开始,延期结束
                    if(Number(item.schedule_start_timestamp) > lastDate && Number(item.end_timestamp) > lastDate){
                        //头尾都在最后时间外
                        ...
                        //尾超 头不超
                        item.startX = _this.findStartIndex(item.schedule_start_timestamp) * _this.cellWidth;
                        item.endX = Math.ceil((((lastDate - Number(item.schedule_start_timestamp)) / 86400)) * _this.cellWidth);
                        ...
                        //都不超
                        item.startX = _this.findStartIndex(item.schedule_start_timestamp) * _this.cellWidth;
                        item.endX = Math.ceil((((Number(item.end_timestamp) - Number(item.schedule_start_timestamp)) / 86400)) * _this.cellWidth);
                        delayStartDay = (Number(item.start_timestamp) - Number(item.schedule_start_timestamp)) / 86400;
                        delayEndDay = (Number(item.end_timestamp) - Number(item.schedule_end_timestamp)) / 86400;
                    }
                    ...
                    //延期开始,提前结束
                    if(Number(item.schedule_start_timestamp) > lastDate && Number(item.schedule_end_timestamp) > lastDate){
                        //头尾都在最后时间外
                        let exceededStartDay = Math.ceil((Number(item.schedule_start_timestamp) - lastDate) / 86400);
                        let exceededEndDay = Math.ceil((Number(item.schedule_end_timestamp) - lastDate) / 86400);
                        item.startX = (_this.findStartIndex(lastDate) + exceededStartDay) * _this.cellWidth;
                        ...
                        //都超
                        item.startX = _this.findStartIndex(item.schedule_start_timestamp) * _this.cellWidth;
                        item.endX = Math.ceil((((Number(item.schedule_end_timestamp) - Number(item.schedule_start_timestamp)) / 86400)) * _this.cellWidth);
                        delayStartDay = (Number(item.start_timestamp) - Number(item.schedule_start_timestamp)) / 86400;
                        advanceEndDay = (Number(item.schedule_end_timestamp) - Number(item.end_timestamp)) / 86400;
                    }
