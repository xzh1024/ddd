        ...
        methods:{
            ...
            calcAdvanceOrDelay(item){
                ...
                if(Number(item.schedule_start_time_unix) < firstDate && Number(item.schedule_end_time_unix) < firstDate){
                    ...
                    //正常开始
                    let startIndex = this.findStartIndex(Number(this.calcTimeFrame(item.schedule_start_time_unix,'start')));
                    let startDay = this.dateTime[startIndex].begin;
                    let afterDay = Math.ceil((Number(this.calcTimeFrame(item.schedule_start_time_unix,'start')) - startDay)/86400);
                    ...
                    if(Number(item.schedule_end_time_unix) === Number(item.end_time_unix)){
                        //正常结束
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.advanceEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.end_time_unix,'end'))) / 86400) * this.dayWidth;
                        item.firstTime = this.coverDateTime(item.schedule_start_time_unix,'dateTime');
                        item.lastTime = this.coverDateTime(item.schedule_end_time_unix,'dateTime');
                    }else if(Number(item.end_time_unix) > Number(item.schedule_end_time_unix)){
                        //延期结束
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.delayEndX = Math.ceil((Number(this.calcTimeFrame(item.end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_end_time_unix,'end'))) / 86400) * this.dayWidth;
                        item.firstTime = this.coverDateTime(item.schedule_start_time_unix,'dateTime');
                    ...
                    //提前开始
                    let startIndex = this.findStartIndex(Number(this.calcTimeFrame(item.start_time_unix,'start')));
                    let startDay = this.dateTime[startIndex].begin;
                    let afterDay = (Number(this.calcTimeFrame(item.start_time_unix,'start')) - startDay)/86400;
                    let taskStartAfterDay = (Number(this.calcTimeFrame(item.schedule_start_time_unix,'start')) - Number(this.calcTimeFrame(item.start_time_unix,'start'))) / 86400;
                    ...
                    if(Number(item.schedule_end_time_unix) === Number(item.end_time_unix)){
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.end_time_unix,'end')) - Number(this.calcTimeFrame(item.start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.delayEndX = Math.ceil((Number(this.calcTimeFrame(item.end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_end_time_unix,'end'))) / 86400) * this.dayWidth;
                        item.firstTime = this.coverDateTime(item.start_time_unix,'dateTime');
                        item.lastTime = this.coverDateTime(item.end_time_unix,'dateTime');
                    ...
                    let startDay = this.dateTime[startIndex].begin;
                    let afterDay = Math.ceil((Number(this.calcTimeFrame(item.schedule_start_time_unix,'start')) - startDay)/86400);
                    let taskStartAfterDay = (Number(this.calcTimeFrame(item.start_time_unix,'start')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400;
                    let startX = Math.ceil((startIndex * this.cellWidth) + (afterDay * this.dayWidth));
                    ...
                    if(Number(item.schedule_end_time_unix) === Number(item.end_time_unix)){
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.firstTime = this.coverDateTime(item.schedule_start_time_unix,'dateTime');
                        item.lastTime = this.coverDateTime(item.schedule_end_time_unix,'dateTime');
                    }else if(Number(item.end_time_unix) < Number(item.schedule_end_time_unix)){
                        //提前结束
                        ...
                        item.endX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.end_time_unix,'end')) - Number(this.calcTimeFrame(item.start_time_unix,'start'))) / 86400) * this.dayWidth;
                        item.advanceEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.end_time_unix,'end'))) / 86400) * this.dayWidth;
                        item.firstTime = this.coverDateTime(item.schedule_start_time_unix,'dateTime');
                        item.lastTime = this.coverDateTime(item.schedule_end_time_unix,'dateTime');
                    }else if(Number(item.end_time_unix) > Number(item.schedule_end_time_unix)){
                        ...
                        if(Number(item.start_time_unix) >= Number(item.schedule_end_time_unix)){
                            item.taskEndX = 0;
                            item.delayEndX = Math.ceil((Number(this.calcTimeFrame(item.end_time_unix,'end')) - Number(this.calcTimeFrame(item.start_time_unix,'start'))) / 86400) * this.dayWidth;
                        }else {
                            item.taskEndX = Math.ceil((Number(this.calcTimeFrame(item.schedule_end_time_unix,'end')) - Number(this.calcTimeFrame(item.start_time_unix,'start'))) / 86400) * this.dayWidth;
                            item.delayEndX = Math.ceil((Number(this.calcTimeFrame(item.end_time_unix,'end')) - Number(this.calcTimeFrame(item.schedule_end_time_unix,'end'))) / 86400) * this.dayWidth;
                        }
            ...
            drawToDay(ctx,flag){
                let dateTime = this.dateTime;
                let len = dateTime.length;
                let lastDate = dateTime[len-1].end + (6*86400); //当前时间段最后一个时间 + 6天的时间 因为是已周为刻度;
                let nowDay = new Date();
