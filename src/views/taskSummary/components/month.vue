    ...
    export default {
        ...
        methods:{
            ...
            calcAdvanceOrDelay(item){
                ...
                if(Number(item.start_time_unix) < Number(item.schedule_start_time_unix) && Number(item.end_time_unix) < Number(item.schedule_end_time_unix)){
                    //提前开始,提前结束
                    let startIndex = _this.findStartIndex(item.start_time_unix);
                    let startDay = _this.dateTime[startIndex].begin;
                    let afterDay = (Number(item.start_time_unix) - startDay)/86400;

                    if(Number(item.start_time_unix) <= firstDate && Number(item.schedule_end_time_unix) <= lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.schedule_end_time_unix) - firstDate)/86400)) * _this.dayWidth);
                        ...
                        item.endX = Math.ceil((((lastDate - Number(item.start_time_unix))/86400)) * _this.dayWidth);
                        ...
                        //都不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((Number(item.schedule_end_time_unix) - Number(item.start_time_unix))/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_time_unix) - Number(item.start_time_unix))/86400;
                        advanceEndDay = (Number(item.schedule_end_time_unix) - Number(item.end_time_unix))/86400;
                    }
                    ...
                    let startDay = _this.dateTime[startIndex].begin;
                    let afterDay = (Number(item.start_time_unix) - startDay)/86400;
                    if(Number(item.start_time_unix) <= firstDate && Number(item.end_time_unix) <=lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.end_time_unix) - firstDate)/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_time_unix) - firstDate)/86400;
                        ...
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((Number(item.end_time_unix) - Number(item.start_time_unix))/86400)) * _this.dayWidth);
                        advanceStartDay = (Number(item.schedule_start_time_unix) - Number(item.start_time_unix))/86400;
                        delayEndDay = (Number(item.end_time_unix) - Number(item.schedule_end_time_unix))/86400;
                    }
                    ...
                    //延期开始,延期结束
                    let startIndex = _this.findStartIndex(item.schedule_start_time_unix);
                    let startDay = _this.dateTime[startIndex].begin;
                    let afterDay = (Number(item.schedule_start_time_unix) - startDay)/86400;

                    if(Number(item.schedule_start_time_unix) <= firstDate && Number(item.end_time_unix) <= lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.end_time_unix) - firstDate)/86400)) * _this.dayWidth);
                        ...
                        //尾超 头不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((lastDate - Number(item.schedule_start_time_unix))/86400)) * _this.dayWidth);
                        ...
                        //都不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((Number(item.end_time_unix) - Number(item.schedule_start_time_unix))/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_time_unix) - Number(item.schedule_start_time_unix))/86400;
                        delayEndDay = (Number(item.end_time_unix) - Number(item.schedule_end_time_unix))/86400;
                    }
                    ...
                    if(Number(item.schedule_start_time_unix) <= firstDate && Number(item.schedule_end_time_unix) <= lastDate){
                        //头超 尾不超
                        item.startX = 0;
                        item.endX = Math.ceil((((Number(item.schedule_end_time_unix) - firstDate)/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_time_unix) - firstDate)/86400;
                        ...
                        //都不超
                        item.startX = (startIndex * _this.cellWidth) + (afterDay * _this.dayWidth);
                        item.endX = Math.ceil((((Number(item.schedule_end_time_unix) - Number(item.schedule_start_time_unix))/86400)) * _this.dayWidth);
                        delayStartDay = (Number(item.start_time_unix) - Number(item.schedule_start_time_unix))/86400;
                        advanceEndDay = (Number(item.schedule_end_time_unix) - Number(item.end_time_unix))/86400;
                    }
                    item.delayStartX = delayStartDay * _this.dayWidth;
